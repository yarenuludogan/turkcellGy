...
Circuit Breaker, mikroservis mimarisinde kullanılan bir hata tolerans (fault tolerance) mekanizmasıdır. Amacı, başarısız olan bir servise sürekli istek gönderilmesini engelleyerek sistemin tamamen çökmesini önlemektir. İsmini elektrik devrelerindeki sigorta mantığından alır. Elektrikte bir devrede sorun olduğunda sigorta devreyi keser; Circuit Breaker da benzer şekilde problemli servise giden istekleri geçici olarak durdurur.

Mikroservislerde servisler birbirine bağımlı çalışır. Örneğin Order Service, Payment Service’e istek atıyorsa ve Payment Service yavaşlamış veya çökmüşse, Order Service sürekli timeout almaya başlar. Eğer bu durum kontrol edilmezse:

- Thread’ler dolabilir
- Sistem yavaşlayabilir
- Zincirleme çökme (cascade failure) oluşabilir.

Circuit Breaker’ın çalışma mantığında genellikle üç durum (state) vardır:

#### 1. Closed State

Normal çalışma durumudur.
İstekler doğrudan hedef servise gider. Sistem hata oranını takip eder.

Örneğin:

100 isteğin 2’si başarısız → normal kabul edilir
#### 2. Open State

Eğer hata oranı belirlenen threshold’u aşarsa circuit “open” olur.

Bu durumda:

Artık hedef servise istek gönderilmez
İstekler direkt reddedilir
Genellikle fallback response döndürülür

Örneğin:

“Servis şu anda kullanılamıyor”
Cache veri döndürme
Varsayılan cevap dönme

Böylece problemli servis daha fazla yük altında kalmaz.

#### 3. Half-Open State

Belirli bir süre sonra Circuit Breaker sistemi tekrar test etmek ister.

Bu durumda:

Az sayıda test isteği gönderilir
Eğer başarılı olursa circuit tekrar closed olur
Hatalar devam ederse yeniden open durumuna geçer
Neden önemlidir?
...
- Timeout yükünü azaltır
- Sistemi daha dayanıklı hale getirir

Özellikle distributed system ve microservice mimarilerinde kritik bir pattern’dir.

Spring ekosisteminde bu amaçla en yaygın kullanılan yapılardan biri Resilience4j’dir. Eskiden Netflix Hystrix çok yaygındı ancak artık deprecated durumdadır.

Özetle Circuit Breaker, başarısız servisleri geçici olarak sistemden izole ederek tüm mimarinin etkilenmesini engelleyen koruyucu mekanizmadır.

### KUBERNATES

Container tabanlı uygulamaları otomatik olarak yönetmek, ölçeklemek ve dağıtmak için kullanılan açık kaynaklı bir container orchestration platformudur. İlk olarak Google tarafından geliştirilmiş, daha sonra CNCF (Cloud Native Computing Foundation) tarafından yönetilmeye başlanmıştır. Özellikle büyük microservice mimarilerinde çok yaygın kullanılır.

Kubernetes’in temel amacı, çok sayıda container’ın düzenli ve otomatik şekilde çalışmasını sağlamaktır. Çünkü modern uygulamalarda onlarca hatta yüzlerce microservice olabilir ve bunların manuel yönetimi oldukça zor hale gelir. Kubernetes bu karmaşıklığı otomatikleştirir.
